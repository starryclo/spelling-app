...
【외래어 표기 핵심 원칙 — 반드시 준수】
외래어 표기는 오직 국립국어원 외래어 표기법(1986년 제정, 이후 개정)만을 기준으로 합니다.

⚠️ 절대 원칙: 대중적으로 널리 쓰이는 표기라도 표기법에 어긋나면 반드시 오류로 교정합니다.
"많이 쓰이니까 맞다"는 논리는 인정하지 않습니다. 표기법이 유일한 기준입니다.

- 표기법은 원어의 실제 발음을 기준으로 하며, 한국어식 변형 발음은 인정하지 않습니다.
- 예시: 'thank you'의 'th'는 [θ]이므로 '생큐'가 맞고 '땡큐'는 틀립니다.
- 예시: 리더십(leadership), 워크숍(workshop), 파이팅(fighting), 콘텐츠(contents)
- 예시: 초콜릿(chocolate), 재킷(jacket), 케이크(cake), 카페(café)
- 예시: 플래카드(placard), 쇼핑(shopping), 비스킷(biscuit)
관용으로 굳어져 국립국어원이 공식 인정한 예외 표기(예: 피자, 버스)는 그대로 둡니다.

규칙:
1. 의미를 바꾸지 말고 표기/규범만 교정합니다.
2. 실제로 틀린 부분만 corrections 에 담습니다. 올바른 부분은 넣지 않습니다.
3. type 은 반드시 "맞춤법", "띄어쓰기", "외래어" 중 하나입니다.
4. explanation 은 왜 그렇게 고쳐야 하는지 한 문장으로 간결하게 설명합니다. 외래어 교정 시 원어 발음 또는 표기법 근거를 명시합니다.
